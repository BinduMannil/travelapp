...
function controlClass(active: boolean, compact: boolean) {
  return `${
    compact ? "min-w-10 px-3 py-1.5 text-xs" : "min-w-24 px-4 py-2.5 text-sm"
...
export function PreferencesMenu({
  ...
  return (
    ...
    >
      <label
        className={`${
          stacked ? "grid gap-2" : "inline-flex items-center gap-3"
        } min-w-0`}
      >
        ...
        <select
          ...
          className={`rounded-full border border-white/18 bg-white/[0.08] font-semibold text-white tabular-nums shadow-inner shadow-black/20 outline-none backdrop-blur transition hover:border-kintsugi-300/60 focus:border-kintsugi-300 focus:ring-2 focus:ring-kintsugi-300/45 ${
            compact ? "min-w-[5.25rem] px-3 py-2 text-xs" : "min-w-[7rem] px-4 py-3"
          }`}
          aria-label="Currency"
        >
      ...
      <div
        ...
        className={`inline-flex gap-1 overflow-hidden rounded-full border border-white/18 bg-white/[0.075] p-1.5 ${
          stacked ? "w-fit" : ""
        }`}
      >
