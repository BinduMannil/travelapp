      ...
      <div className="mx-auto flex h-16 max-w-[1920px] items-center gap-3 px-4 sm:px-6">
        ...
        <nav className="hidden flex-1 items-center justify-center gap-0.5 2xl:flex">
          {navItems.map((item) => (
            <Link
              key={item}
              href={item === "Home" ? "/" : item === "Map" ? "/atlas" : `/${item.toLowerCase()}`}
              className={`relative px-3 py-5 text-sm font-medium transition 2xl:px-3.5 ${
                item === "Alerts" ? "text-[#f3b432]" : "text-white/88 hover:text-white"
              }`}
            >
              {item}
              {item === "Alerts" && (
                <span className="absolute bottom-0 left-3 right-3 h-0.5 rounded-full bg-[#f3b432]" />
              )}
            </Link>
          ))}
        </nav>
